...
def test_process_uploaded_audio_mp3(monkeypatch, tmp_path) -> None:
    """Audio files (e.g. MP3) should be processed the same way as video files."""
    audio = tmp_path / "recording.mp3"
    audio.write_bytes(b"binary-audio")
    ...
    assert result["job_id"] == "audio123"
    assert Path(result["source_path"]).resolve() == audio.resolve()
    audio_path = Path(result["audio_path"])
    assert audio_path.exists()
