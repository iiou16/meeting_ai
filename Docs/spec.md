...
- **目的**: 会議の動画・音声ファイルから自動的に議事録を生成し、参加者が重要ポイントとアクションアイテムをすばやく把握できるようにする。
- **想定ユーザー**: 社内で定期的にオンライン会議を行うチーム、録画を共有する必要があるプロジェクトマネージャー。
- **提供価値**: 非参加者でも短時間で内容を理解できる要約と、タイムスタンプ付きで原文にアクセスできる精度の高い文字起こし。

## 2. 処理フロー

1. フロントエンドが動画ファイル(MP4/Mov/WebM等)または音声ファイル(MP3/WAV/M4A/AAC/FLAC/OGG等)、もしくは動画URLを受け取り、バックエンドAPIへ送信。
2. Pythonバックエンドが入力ファイルを判定し、動画の場合は音声トラックを抽出、音声ファイルの場合はそのまま取り込み、WAV形式(モノラル・16kHz)に統一変換。
...
- **メディア入力**
  - 動画ファイルアップロード: MP4, MOV, WebM等（最大長さ・サイズ制限を設定）。
  - 音声ファイルアップロード: MP3, WAV, M4A, AAC, FLAC, OGG等。
  - URLインポート（クラウドストレージや会議録画リンク）。ダウンロード処理は非同期。
- **音声抽出・前処理**
  - 動画ファイルの場合: FFmpeg等で動画から音声のみ抽出。
  - 音声ファイルの場合: FFmpegでWAV形式（モノラル・16kHz・PCM 16-bit）に統一変換。
  - 長時間ファイルに対応するための分割（例: 15分単位）とサンプリングレート変換。
- **文字起こし**
...
- `POST /api/videos`: 動画・音声ファイルアップロード。レスポンスに処理ジョブID。
