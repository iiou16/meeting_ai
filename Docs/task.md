...
- [ ] `_has_source_video` を音声ファイル拡張子にも対応させる（現状は動画のみ検出。`_detect_stage` のフォールバックで動作はするが、正確なステージ判定のため対応が望ましい）。
...
- [ ] `ingest.py` の `enqueue_transcription_job` 失敗時にジョブを失敗扱いとして記録する（現状は `logger.exception` でログ出力のみで処理続行しており、ジョブ進捗管理との整合性に懸念あり）。
